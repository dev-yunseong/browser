...
| The `place-*` shorthands never expanded | `place-content`, `place-items` and `place-self` set both axes at once — one value for both, two for the block axis then the inline one. Reading only the `justify-*` longhand left github's "Explore customer stories" link hard against the section's left padding where the reference centres it across the page. |
...
| The same width never read off the ratio when *measuring* a parent | Layout took the height and the ratio; the intrinsic-size pass went straight to the 100px replaced placeholder. A box that shrinks to fit an icon came out 84px too wide, so github's language button and the span around its chevron were sized for a placeholder rather than the 16px icon they hold. |
...
|---|---|---|---|---|
| github.com | 0.46% | 1.10% | 1.44% | 10570 -> 10553 |
